...
class ProviderModeListener
{
    ...
    private function getClients(array $options = []): array
    {
        ...
        if ($this->settings->get('abdulrehman-sso.provider_mode') === '1') {
            ...
            for ($i = 1; ($clientUrl = $this->settings->get("abdulrehman-sso.client{$i}_url")) !== null; $i++) {
                ...
                $clients[] = new Flarum(array_merge([
                    ...
                    'api_key' => $this->settings->get("abdulrehman-sso.client{$i}_api_key"),
                    'password_token' => $this->settings->get("abdulrehman-sso.client{$i}_password_token"),
                    'verify_ssl' => $this->settings->get("abdulrehman-sso.client{$i}_verify_ssl") === '1',
                    'cookies_prefix' => $this->settings->get("abdulrehman-sso.client{$i}_name", $clientUrl)
                ], $options));
            }
        }
        return $clients;
